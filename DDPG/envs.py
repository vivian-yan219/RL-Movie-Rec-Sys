...
class OfflineEnv(object):
    ...
    def reset(self):
        ...
        for data in self.users_dict[self.user]:
            ...
            if movie_id not in seen:
                self.items.append(movie_id)
                seen.add(movie_id)
            if len(self.items) == self.state_size:
                break
    ...
    def step(self, action, top_k=False):
        ...
        if top_k:
            ...
            rewards = []
            for act in action:
                act = int(act)  # Ensure that the action is hashable (e.g., int)
        ...
        # test set
        if (len(self.recommended_items) > self.done_count) or (len(self.recommended_items) >= self.users_history_lens[self.user-488-1]):
            self.done = True
